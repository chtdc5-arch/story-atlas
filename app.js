(() => {
  ...
  function openWork(existing=null){ const w=existing||{};modal(existing?'編輯作品':'建立作品','作品是所有話數、角色、場景與資產的關聯中心。',`<div class="form-grid">${field('title','書名',w.title)}${field('status','狀態',w.status||'構思中')} ${field('summary','簡介',w.summary,true,true)}</div>`,(root,close)=>{const title=root.querySelector('[name=title]').value.trim();if(!title)return toast('請輸入書名');if(existing){Object.assign(existing,{title,summary:root.querySelector('[name=summary]').value,status:root.querySelector('[name=status]').value,updatedAt:stamp()});}else{const item={id:uid('work'),title,summary:root.querySelector('[name=summary]').value,status:root.querySelector('[name=status]').value,updatedAt:stamp(),episodes:[],characters:[],scenes:[],assets:[]};state.works.push(item);state.selectedWorkId=item.id;}save();close();render();toast(existing?'作品已更新':'作品已建立');}); }
  function openEpisode(existing=null){ const w=work();if(!w)return toast('請先選擇作品');const e=existing||{};modal(existing?'編輯話數':'新增話數','建立後可在寫作內容中編輯四種版本。',`<div class="form-grid">${field('title','話數標題',e.title)}${field('status','內容狀態',e.status||'構思中')}${field('outline','大綱',e.outline,'full',true)}</div>`,(root,close)=>{const title=root.querySelector('[name=title]').value.trim();if(!title)return toast('請輸入話數標題');if(existing){Object.assign(existing,{title,status:root.querySelector('[name=status]').value,outline:root.querySelector('[name=outline]').value,updatedAt:stamp()});}else{const item={id:uid('episode'),no:w.episodes.length+1,title,status:root.querySelector('[name=status]').value,outline:root.querySelector('[name=outline]').value,novel:'',comic:'',video:'',versions:[{version:1,note:'建立話數',at:stamp()}]};w.episodes.push(item);activeEpisodeId=item.id;state.view='episodes';}save();close();render();toast(existing?'話數已更新':'話數已建立');}); }
