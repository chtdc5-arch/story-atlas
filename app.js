(() => {
  ...
  function episodeFormatBody(ep){
    const textEditor=editorMode==='write'?`<textarea id="contentEditor" placeholder="可補充漫畫製作備註">${esc(ep[editorFormat]||'')}</textarea>`:`<article class="markdown-preview">${markdown(ep[editorFormat]||'尚未輸入內容')}</article>`;
    if(editorFormat!=='comic')return textEditor;
    const pages=Array.isArray(ep.comicPages)?ep.comicPages:[];
    const sequence=pages.length?`<section class="comic-sequence"><div class="comic-sequence-head"><div><p class="eyebrow">COMIC PAGES</p><h3>本話漫畫</h3></div><div class="comic-sequence-meta"><span>${pages.length} 頁・依 P01 → P${String(pages.length).padStart(2,'0')} 閱讀</span><a class="comic-reader-link" href="comic.html?chapter=${ep.no}">獨立閱讀 ↗</a></div></div><div class="comic-page-grid">${pages.map((page,index)=>{const item=typeof page==='string'?{src:page}:page;const label=item.label||`P${String(index+1).padStart(2,'0')}`;return `<button class="comic-page has-cover" data-preview-image="${esc(item.src)}" data-preview-title="${esc(`第 ${ep.no} 話・${label}`)}"><img src="${esc(item.src)}" alt="第 ${ep.no} 話漫畫 ${esc(label)}" loading="lazy"><span>${esc(label)}</span></button>`;}).join('')}</div></section>`:`<div class="empty comic-empty"><div><strong>本話尚未放入漫畫頁</strong><small>漫畫頁完成後，可使用每話獨立閱讀模式。</small><a class="comic-reader-link" href="comic.html?chapter=${ep.no}">開啟漫畫閱讀頁 ↗</a></div></div>`;
    return `${sequence}<details class="comic-notes" ${ep[editorFormat]?'open':''}><summary>漫畫製作備註</summary>${textEditor}</details>`;
  }
  function episodes(){
    ...
    const side=eps.length?`<div class="episode-list"><div class="list-head"><strong>全部話數</strong><span>${eps.length}</span></div>${eps.map((e,i)=>`<button class="episode-row ${e.id===activeEpisodeId?'selected':''}" data-episode="${e.id}"><b class="episode-cover ${e.coverUrl?'has-cover':''}" data-preview-image="${e.coverUrl||''}" data-preview-title="${esc(e.title)}" ${e.coverUrl?`style="background-image:url('${e.coverUrl}')"`:''}>${e.coverUrl?'':String(i+1).padStart(2,'0')}</b><span><strong>${esc(e.title)}</strong><small>${esc(e.status||'構思中')}・第 ${e.versions?.length||1} 版</small></span><i>⋯</i></button>`).join('')}</div>`:emptyBlock('尚未建立話數','按右上角「新增話數」開始。');
    const editor=ep?`<div class="editor"><div class="tabs">${[['outline','大綱'],['novel','網路小說'],['comic','漫畫'],['video','影片']].map(([k,l])=>`<button class="${editorFormat===k?'active':''}" data-format="${k}">${l}</button>`).join('')}</div><div class="editor-body"><div class="editor-head"><span class="number">第 ${ep.no} 話</span><h2>${esc(ep.title)}</h2><span class="version">V${ep.versions?.[0]?.version||1}</span><button class="quiet mini" data-edit-episode="${ep.id}">編輯話數</button></div><div class="editor-mode"><button class="${editorMode==='write'?'active':''}" data-editor-mode="write">編輯原文</button><button class="${editorMode==='preview'?'active':''}" data-editor-mode="preview">預覽格式</button><small>Obsidian Markdown：**粗體**　1. 編號　- 清單　# 標題　&gt; 引用　[[連結]]</small></div>${episodeFormatBody(ep)}<div class="editor-footer"><small>本機保存・每次儲存會建立版本</small><button class="quiet" data-history>版本紀錄</button>${editorMode==='write'?'<button class="primary" data-save-content>儲存新版本</button>':''}</div></div></div>`:emptyBlock();
  ...
  function bindView(){
    document.querySelectorAll('[data-preview-image].has-cover').forEach(el=>el.onclick=e=>{e.stopPropagation();previewImage(el.dataset.previewImage,el.dataset.previewTitle);});
  ...
  function previewImage(src,title='圖片預覽'){const root=document.querySelector('#modalRoot');root.innerHTML=`<div class="image-lightbox"><button class="image-lightbox-close" aria-label="關閉">×</button><img src="${src}" alt="${title}"><small>${title}</small></div>`;const close=()=>root.innerHTML='';root.querySelector('.image-lightbox-close').onclick=close;root.querySelector('.image-lightbox').onclick=e=>{if(e.target===e.currentTarget)close();};}
  ...
  function readImage(root,name,current=''){const file=root.querySelector(`[name=${name}]`)?.files?.[0];if(!file)return Promise.resolve(current);return new Promise(resolve=>{const reader=new FileReader();reader.onload=()=>{const img=new Image();img.onload=()=>{const max=1400,scale=Math.min(1,max/Math.max(img.naturalWidth,img.naturalHeight)),canvas=document.createElement('canvas');canvas.width=Math.max(1,Math.round(img.naturalWidth*scale));canvas.height=Math.max(1,Math.round(img.naturalHeight*scale));canvas.getContext('2d').drawImage(img,0,0,canvas.width,canvas.height);resolve(canvas.toDataURL('image/jpeg',.82));};img.onerror=()=>resolve(reader.result);img.src=reader.result;};reader.readAsDataURL(file);});}
